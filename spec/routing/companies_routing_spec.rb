require "rails_helper"

RSpec.describe CompaniesController, type: :routing do
  describe "routing" do
    it "routes to #index" do
      expect(get: "/api/v1/companies").to route_to("companies#index")
    end

    it "routes to #vendor_listings" do
      expect(get: "/api/v1/companies/vendor_listings").to route_to("companies#vendor_listings")
    end

    it "routes to #clients" do
      expect(get: "/api/v1/companies/1/clients").to route_to("companies#clients", company_id: "1")
    end

    it "routes to #show" do
      expect(get: "/api/v1/companies/1").to route_to("companies#show", id: "1")
    end

    it "routes to #create" do
      expect(post: "/api/v1/companies").to route_to("companies#create")
    end

    it "routes to #update via PUT" do
      expect(put: "/api/v1/companies/1").to route_to("companies#update", id: "1")
    end

    it "routes to #update via PATCH" do
      expect(patch: "/api/v1/companies/1").to route_to("companies#update", id: "1")
    end

    it "routes to #destroy" do
      expect(delete: "/api/v1/companies/1").to route_to("companies#destroy", id: "1")
    end
  end
end
