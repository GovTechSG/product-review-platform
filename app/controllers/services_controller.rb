class ServicesController < ApplicationController
  ...
  def find_service
    if Service.find_by_name(params[:service_name]).nil?
      # Service.create()
      @service_id = {'service_id':'need to create'}
      render json: {'service_id':Service.first.hashid}
    else
      render json: {'service_id':Service.find_by_name(params[:service_name]).hashid }
    end
  end
